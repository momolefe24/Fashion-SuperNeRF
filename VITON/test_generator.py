...
def get_opt():
    ...
    parser.add_argument('--tensorboard_dir', type=str, default='./data/rail/tensorboard', help='save tensorboard infos')
    ...
    with torch.no_grad():
        for inputs in test_loader.data_loader:
            if opt.cuda :
                ...
                transform_matrix = inputs['transform_matrix'].cuda()
                ...
                label = inputs['parse']
                transform_matrix = inputs['transform_matrix']
                parse_agnostic = inputs['parse_agnostic']
...
def main():
    ...
    to_numpy = lambda img: img.permute(1,2,0).cpu().numpy()
    # visualization
